/*
 * XChaCha20UdonStatic.cs
 * ─────────────────────────────────────────────────────────────────────────────
 * VRChat UdonSharp — XChaCha20 Encryption / Decryption (static utility)
 *
 * RFC compliance
 ...
 *   string cipher = XChaCha20UdonStatic.Encrypt("Hello VRChat!", "my-secret-pass");
 *   string plain  = XChaCha20UdonStatic.Decrypt(cipher, "my-secret-pass");
 * ─────────────────────────────────────────────────────────────────────────────
 */

using UnityEngine;

public static class XChaCha20UdonStatic
{
    // ═══════════════════════════════════════════════════════════════════════
    //  PUBLIC API
    // ═══════════════════════════════════════════════════════════════════════
    ...
    private static byte[] XChaCha20Process(byte[] data, byte[] key, byte[] nonce24,
                                    uint initialCounter)
    {
    ...
    private static byte[] ChaCha20Process(byte[] data, byte[] key, byte[] nonce12,
                                   uint counter)
    {
    ...
    private static byte[] ChaCha20KeystreamBlock(byte[] key, byte[] nonce12,
                                          uint counter)
    {
        uint[] state = new uint[16];
    ...
    private static void ChaCha20Block20Rounds(uint[] s)
    {
        for (int i = 0; i < 10; i++)
        {
            // Column rounds
    ...
    private static void MulU64(ref uint hi, ref uint lo, uint phi, uint plo)
    {
    ...
    private static void Xorshift64(ref uint hi, ref uint lo)
    {
    ...
    private static int B64CharVal(char c)
    {
    ...
    private static void PutLE32(uint v, byte[] buf, int offset)
    {
    ...
    private static int GetCodePoint(string s, ref int i)
    {
        char c = s[i++];
        // High surrogate?
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //  QUICK SELF-TEST  (call from within a UdonSharp script)
    // ═══════════════════════════════════════════════════════════════════════

    public static void SelfTest(string textToEncrypt = "This is the data string that needs to be encrypted", string defaultPassword = "VRChatSecretKey!")
    {
